...
2. Create a code chunk that
  ...
  + Sets a global chunk option to make all figures 6.5" wide and the height to
    a value that makes sense to you, and
...
3. Import the *ecls-k_samp.sav* dataset (stored on Canvas in *Files -> data*), and produce the plot below. Do not show
   the code you used (colors, themes, etc. don't matter here).

```{r load-data, echo = FALSE}

eclsk <- import("C:/Users/Joe/Desktop/BRT/Teaching/Intro-Data-Science/2019_fall/labs/data/ecls-k_samp.sav") %>% 
#  import(here("data", "ecls-k_samp.sav"), setclass = "tbl_df") %>%
  characterize() %>%
  clean_names() 
...
4. Run The following lines of code to store the mean and standard deviation of
  ...
  **ecls-k** dataset in as an object called `eclsk`. You should substitute in whatever
...
the *means* and *SD*s for the two time points (`t1mscale` and `t2mscale`). Also report the difference between the
